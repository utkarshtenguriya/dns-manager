...
const createRecord = asyncHandler(async (req: Request, res: Response) => {
    const command = generateCreateRecordCommand(req.body);
    ...
    try {
        ...
        if (flag) {
            data = (
                await client.send(
                    generateListRecordSetsCommand(req.body.hosted_zone_id)
                )
            ).ResourceRecordSets;
        }
        return res
...
const deleteRecord = asyncHandler(
    async (req: TypedRequest<RecordBody>, res: Response) => {
        ...
        const command = generateDeleteRecordCommand(req.body);
...
const fetchRecord = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        const command = generateListRecordSetsCommand(hosted_zone_id);
...
const updateRecord = asyncHandler(
    async (req: TypedRequest<RecordBody>, res: Response) => {
        // extracting requesting body data
        const { Name, hosted_zone_id } = req.body;
        ...
        try {
            ...
            let flag = await client.send(
                generateDeleteRecordCommand(oldData)
            );
            ...
            // create new record 
            flag = await client.send(generateCreateRecordCommand(req.body));
